...
function Logo() {
  return (
    <div className="font-bold text-xl text-blue-600 dark:text-blue-500">
      <Link href='/'><a className='hover:text-blue-500 dark:hover:text-blue-600' >Thada W.</a></Link>
    </div>
  )
}
...
function Menu({ href, children}: IMenuProps) {
  return (
    ...
    <Link href={href} >
      <a className=" hover:bg-slate-100 hover:text-slate-900 dark:hover:text-slate-50 dark:hover:bg-gray-800 px-4 py-2 rounded-md text-md font-medium">
        {children}
      </a>
    </Link>
    // </div>
